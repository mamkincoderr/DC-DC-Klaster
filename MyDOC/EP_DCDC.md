...
**Проверка размера сердечника.** Способность сердечника отдать эту мощность при заданных частоте и плотности тока обмоток выражается через произведение сечения на окно (площадь-произведение):

$$ {\Large S_{\text{т}} = f \cdot B_{\text{max}} \cdot A_e \cdot A_w \cdot K_u \cdot J } ,~(2.1)$$

где Ku ≈ 0,3 это коэффициент заполнения окна медью с учётом изоляции и зазоров, J плотность тока обмотки. Для провода при обычной плотности 4 А/мм² и Bmax = 230 мТл (типовое рабочее значение для N87 на этой частоте, порядок величины ниже насыщения ~390 мТл при 100 °C) формула (2.1) даёт около 530 ВА, заметно меньше требуемых 1250 ВА. Разрыв не говорит о нехватке сердечника: формула предполагает одну плотность тока на обе обмотки, а у нас обмотка НВ выполнена фольгой платы, где плотность тока в разы выше провода (для 25 А и сечения витка 0,19 мм² это 130 А/мм², расчёт ниже). Такая плотность допустима именно потому, что фольга тонкая и лежит на диэлектрике платы, отводящем тепло по всей площади витка, но она же и объясняет, почему потери в меди обмотки НВ оказываются главной составляющей баланса. Точная проверка по реальным токам и напряжению даёт S_т = 1300 ВА, что и принимается как расчётное значение.
...
$$ {\Large P_{\text{Cu}} = I^2 \cdot R, \quad R = \rho \cdot \dfrac{L}{A} } ,~(2.3)$$
...
$$ {\Large P_v = P_{v0} \cdot \left(\dfrac{f}{f_0}\right)^{x} \cdot \left(\dfrac{B_{\text{max}}}{B_0}\right)^{y} } ,~(2.4)$$
