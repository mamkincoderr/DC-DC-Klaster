...
Расчёт идёт по цепочке: мощность → диапазон частот → конструкция обмоток → плотность тока → разделение на два трансформатора → выбор сердечника → габаритная мощность → индукция → подтверждение частоты → витки → потери.

**Мощность.** 2000 Вт номинальных, из них на стороне ВВ — 6,6 А действующего тока моста (п. 2.1.1), на стороне НВ — до 50 А суммарного тока обмоток (п. 1.1.1). Разница почти на порядок — следствие коэффициента трансформации, а не потерь.
...
**Обмотки.** Вторичная обмотка (6 витков на трансформатор, см. ниже) выполнена печатной дорожкой в окне сердечника: мало витков, большой ток — то, что удобно и повторяемо на плате, без ручной намотки толстым проводом. Первичная обмотка (23 витка, малый ток) выполнена проводом — печатной дорожкой такое количество витков не разместить без многослойной платы.

**Плотность тока и разделение на два.** Дорожка в окне на двухсторонней плате практически ограничена по току — около 30 А (эмпирическая формула IPC-2221 для наружного слоя даёт близкую цифру, см. ниже). Суммарный ток вторичной стороны — 50 А, в одну дорожку не укладывается. Отсюда разделение трансформатора на два (рисунок 2.1): по 25 А на обмотку, запас 17 % от предела. Первичные обмотки включены последовательно в диагональ моста ВВ (один и тот же ток 6,6 А через обе, напряжение делится пополам), вторичные — каждая на свой мост НВ, мосты запараллелены по LV+/LV−, деление тока между трансформаторами самовыравнивается схемой, отдельного контроля не требует.
...
**Витки.** Коэффициент трансформации одного трансформатора вдвое меньше найденного в п. 1.1.1 коэффициента n = 7,8125 для системы в целом (формула (1.1)) — каждая первичка видит половину напряжения ВВ, а вторичка должна воспроизвести полное напряжение НВ: n_т = n/2 = 3,906. Число витков находится по уравнению трансформатора для прямоугольного напряжения:

$$ {\Large U = 4 \cdot f \cdot N \cdot B_{\text{max}} \cdot A_e } ,~(2.2)$$

Отправная точка — вторичная обмотка, поскольку у неё меньше витков и округление до целого числа заметнее. Рассмотрены три варианта:

Таблица 2.4 — Выбор числа витков вторичной обмотки (f = 60 кГц)

| N2 | N1 (= round(n_т·N2)) | n_т факт. | Bmax при 60 В | Запас до насыщения (390 мТл) | Потери в меди, обе обмотки |
|:--|:--|:--|:--|:--|:--|
| 4 | 16 | 4,000 | 406 мТл | превышение — отклонён | — |
| 5 | 20 | 4,000 | 325 мТл | 17 % | 20,6 Вт |
| **6** | **23** | **3,833** | **271 мТл** | **31 %** | **24,6 Вт** |

При N2 = 4 индукция на верхней границе напряжения батареи (60 В) превышает индукцию насыщения — вариант отпадает сразу. N2 = 5 укладывается, но с запасом всего 17 % — с учётом разброса Bsat и нагрева это мало. N2 = 6 стоит на 4 Вт больше по меди (около 0,2 % от 2000 Вт — пренебрежимо), зато запас по насыщению почти вдвое больше. **Выбор: N2 = 6, N1 = 23.**
...
Индукция на первичке (U1 = U_ВВ/2, N1 = 23) по всему диапазону ВВ:
...
Согласуется с расчётом по вторичке (217 мТл на 51,2 В против 231 мТл там же) — небольшая разница из-за округления витков, в пределах точности первого приближения.

**Потери в меди.** Ширина печатной дорожки вторички подобрана по эмпирической формуле IPC-2221 для наружного слоя (перегрев ΔT = 30 °C):

$$ {\Large I = k \cdot \Delta T^{0.44} \cdot A^{0.725} } ,~(2.3)$$

где k = 0,048, A — сечение в мил². Для I2 = 25 А, медь 2 oz — сечение 0,46 мм², ширина около 6,5 мм (укладывается в окно бобины с учётом зазоров). Провод первички — по плотности тока 4 А/мм² при I1 = 6,6 А: сечение 1,65 мм² (около AWG15). Сопротивление витка — по реальной длине витка бобины (62 мм, даташит) и удельному сопротивлению меди при ≈100 °C:

$$ {\Large P_{\text{Cu}} = I^2 \cdot R, \quad R = \rho \cdot \dfrac{L}{A} } ,~(2.4)$$

Таблица 2.5 — Потери в меди, на один трансформатор (N2=6, N1=23)

| Обмотка | Витков | I, А | Сечение | R, мОм | P_Cu, Вт |
|:--|:--|:--|:--|:--|:--|
| Первичная (провод) | 23 | 6,6 | 1,65 мм² | 19,5 | 0,8 |
| Вторичная (дорожка) | 6 | 25 | 0,46 мм² | 18,3 | 11,5 |
| **Сумма на трансформатор** | | | | | **12,3** |
| **Сумма на два трансформатора** | | | | | **24,6** |

Провод даёт на порядок меньше потерь, чем дал бы аналогичный расчёт печатной дорожкой (толщина меди у провода не ограничена платой) — отсюда и решение мотать первичку проводом, а не дорожкой.
