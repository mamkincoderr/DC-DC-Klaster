...
**Мощность.** 2000 Вт номинальных, из них на стороне ВВ — 6,6 А действующего тока моста (п. 2.1.1), на стороне НВ — до 50 А суммарного тока обмоток (п. 1.1.1). Разница почти на порядок — следствие коэффициента трансформации, а не потерь. Ток вторичной стороны велик для одной обмотки на плате, поэтому предлагается разделить трансформатор на два (рисунок 2.1), по половине мощности и тока на каждый — обоснование ниже, после выбора конструкции обмоток.
...
**Обмотки.** Вторичная сторона — мало витков и большой ток, поэтому обмотку предлагается выполнить печатной дорожкой в окне сердечника: удобно и повторяемо на плате, без ручной намотки толстым проводом. Первичная сторона — наоборот, много витков при небольшом токе; для нее предлагается провод, поскольку такое число витков дорожкой без многослойной платы не разместить.

**Плотность тока.** Дорожка в окне на двухсторонней плате практически ограничена по току — около 30 А (эмпирическая формула IPC-2221 для наружного слоя даёт близкую цифру, см. ниже). Суммарный ток вторичной стороны 50 А в одну такую дорожку не укладывается — это и подтверждает разделение на два трансформатора: по 25 А на обмотку, запас 17 % от предела. Первичные обмотки включены последовательно в диагональ моста ВВ (один и тот же ток 6,6 А через обе, напряжение делится пополам), вторичные — каждая на свой мост НВ, мосты запараллелены по LV+/LV−, деление тока между трансформаторами самовыравнивается схемой, отдельного контроля не требует.
