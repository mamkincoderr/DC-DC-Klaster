...
В двухтактных немостовых и мостовых немостовых DC-DC без резонансного контура — к этому классу относится и рассматриваемый DAB (п. 1.1.5) — сердечник возбуждается двусторонне на постоянной частоте, и в диапазоне 50…100 кГц, типичном для этой мощности и кремниевых ключей (см. частоту 65 кГц, уже фигурировавшую в п. 1.1.3), передаваемая через сердечник мощность при фиксированных f, Bmax и плотности тока обмоток растёт вместе с сечением магнитопровода Ae. Точнее эта зависимость выражается через произведение Ae·Aw (сечение задаёт вольт-витки по закону Фарадея, окно — допустимый ток по сечению меди), но внутри одной серии стандартных сердечников (здесь — PQ) окно масштабируется вместе с сечением достаточно предсказуемо, поэтому сравнение по Ae — оправданное первое приближение для выбора типоразмера. Именно это допущение уже использовано при построении удельной стоимости (цент/мм²) в п. 2.1.2 ниже: если бы Ae не определял мощность хотя бы приближённо, сравнение цены сердечников по сечению было бы бессмысленным.

**Обмотки печатной платой и разделение трансформатора на два.** Мощность 2000 Вт при передаче через изолирующий каскад распределяется между сторонами существенно неравномерно по току: на стороне ВВ действующий ток моста — 6,6 А (п. 2.1.1, из моделирования), на стороне НВ суммарный ток обмоток при полной мощности достигает 50 А (п. 1.1.1). Выполнение вторичной обмотки печатной дорожкой в окне сердечника рационально с точки зрения повторяемости, теплоотвода через полигон платы и исключения ручной намотки — но дорожка в окне на двухсторонней плате практически ограничена по току: около 30 А для сечения, которое ещё вписывается в окно типоразмеров этого класса вместе с изоляционными зазорами. Один трансформатор с одной вторичной обмоткой на 50 А в это ограничение не укладывается — потребовались бы либо дополнительные слои платы под окном, либо дорожка шире доступного окна.

Отсюда — разделение трансформатора на два (рисунок 2.1): каждый несёт половину мощности, и по 25 А на вторичную обмотку — с запасом 17 % под ограничением в 30 А на дорожку, без перехода на многослойную намотку в окне. Первичные обмотки включены последовательно в диагональ моста ВВ (тот же ток 6,6 А через обе, напряжение делится пополам), а вторичные — каждая на собственный мост НВ, оба моста запараллелены по шинам LV+/LV−, так что деление тока между трансформаторами самовыравнивается по построению схемы (п. 2, рисунок 2.1), а не требует отдельного контроля. Разделение по первичке пополам напряжения одновременно снижает требуемое число витков первичной обмотки каждого трансформатора вдвое относительно гипотетического одиночного исполнения — см. расчёт витков ниже.

**Частота коммутации.** В диапазоне 50…100 кГц выбрана частота **65 кГц** — та же величина, что использована как иллюстративная в п. 1.1.3. При меньшей частоте требуемое сечение (или число витков) растёт, а вместе с ним — цена и габариты магнитопровода; при большей частоте пропорционально растут потери в феррите (см. расчёт ниже, показатель степени по частоте у N87 около 1,5) и коммутационные потери в ключах, для которых заряд Qrr и время trr уже были определяющим критерием отбора (п. 2.1.1). 65 кГц — середина рабочего диапазона, не требующая уточнения выбора ключей и позволяющая получить у PQ32/30 рабочую индукцию с комфортным запасом от насыщения (см. ниже).

**Выбор магнитопровода — сопоставление источников цен.** Стоимость сердечников серий EE/E и PQ без зазора проверена по трём независимым источникам — раздельно, без смешивания (см. `DC-DC/Ferrites/ferrite_cores_data.md`):
...
![Удельная стоимость — DigiKey/Mouser/Farnell](PIC/ferrite_specific_cost_digikey.png)

Рисунок 2.2 — Удельная стоимость по сечению, источник DigiKey/Mouser/Farnell

![Удельная стоимость — LCSC (Китай)](PIC/ferrite_specific_cost_china.png)

Рисунок 2.3 — Удельная стоимость по сечению, источник LCSC.com (точные коды TDK N87)

![Удельная стоимость — ChipDip.ru](PIC/ferrite_specific_cost_chipdip.png)

Рисунок 2.4 — Удельная стоимость по сечению, источник ChipDip.ru

Два источника из трёх сходятся на PQ32/30 как минимуме удельной стоимости в серии PQ; третий (китайская оптовая площадка) даёт PQ26/25, но с отрывом всего около 15 % (0,79 против 0,91 цент/мм² — таблицы в `ferrite_cores_data.md`), что не перекрывает практических аргументов ниже. К цене добавляется довод, не связанный со стоимостью напрямую: **PQ32/30 уже массово применяется в каскадах переменного тока стабилизаторов «Килогерц»** (см. Введение) — это действующая позиция закупки с проверенным поставщиком, а не новая номенклатура. Совокупно — цена (два источника из трёх), освоенность в производстве и результат расчёта витков ниже (комфортный запас по индукции) — **выбор: PQ32/30, материал N87**, Ae = 153,8 мм², Ve = 10 440 мм³ (за комплект).

**Расчёт числа витков.** Для трансформатора, включённого по схеме рисунка 2.1 (первички последовательно, вторички раздельные на параллельных мостах), коэффициент трансформации одного трансформатора вдвое меньше коэффициента n = 7,8125, найденного в п. 1.1.1 для системы в целом (формула (1.1)) — потому что каждая первичка видит только половину напряжения ВВ, а каждая вторичка должна воспроизвести полное напряжение НВ:

$$ {\Large n_{\text{т}} = \dfrac{n}{2} } ,~(2.1)$$

n_т = 7,8125 / 2 = 3,906. При заданной вторичной обмотке N2 = 6 витков идеальное число витков первички N1 = 3,906 × 6 = 23,4 — округляется до **N1 = 23**. Фактическое n_т = 23/6 = 3,833, эффективный коэффициент системы n_эфф = 7,667 вместо идеальных 7,8125 (расхождение 1,9 %). Диапазон d в окне полной мощности смещается с 0,84…1,24 (п. 1.1.1) до 0,82…1,21; пересчёт по формулам (1.6)–(1.7) даёт границы ZVS 15,7° (мост ВВ) и 16,2° (мост НВ) вместо 17,2° и 14,8° — обе по-прежнему более чем вдвое меньше рабочего угла 35…37° (п. 1.1.5), запас не нарушен.

Проверка по уравнению трансформатора (закон Фарадея для прямоугольного напряжения):

$$ {\Large U_1 = 4 \cdot f \cdot N_1 \cdot B_{\text{max}} \cdot A_e } ,~(2.2)$$

При U1 = U_ВВ/2 индукция на границах диапазона ВВ (360…430 В):
...
Индукция насыщения N87 при 100 °C — около 390 мТл; рабочая точка 196…234 мТл оставляет запас не менее 40 % во всём диапазоне ВВ, включая перегрузочные выбросы.

**Габаритная (типовая) мощность.** Для проверки соответствия выбранного сердечника нагрузке использована сумма полу-произведений напряжений и токов обмоток одного трансформатора (1000 Вт передаваемой мощности каждый, при действующих токах 6,6 А по первичке и 25 А по вторичке — половина суммарного тока НВ):

$$ {\Large S_{\text{т}} = \dfrac{U_1 I_1 + U_2 I_2}{2} } ,~(2.3)$$

S_т = (200×6,6 + 51,2×25) / 2 = (1320 + 1280) / 2 = **1300 ВА** на трансформатор. Отношение S_т к реальной мощности (1300/1000 = 1,3) отражает не КПД, а реактивную составляющую тока при фазовом управлении (п. 1.1.5) — часть действующего тока не переносит активную мощность.

**Потери в меди.** Ширина печатной дорожки в окне подобрана по эмпирической формуле IPC-2221 для наружного слоя (перегрев ΔT = 30 °C):

$$ {\Large I = k \cdot \Delta T^{0.44} \cdot A^{0.725} } ,~(2.4)$$

где k = 0,048, A — сечение дорожки в мил². Для вторичной обмотки (I2 = 25 А, медь 2 oz) сечение получается 0,457 мм² при ширине дорожки около 6,5 мм — в пределах типового окна бобины PQ32/30 (порядка 7…8 мм) с учётом изоляционных зазоров, что и подтверждает принятую ранее оценку в 30 А на дорожку (см. выше). Для первичной обмотки (I1 = 6,6 А, медь 1 oz, 23 витка) сечение — 0,073 мм² при ширине около 2,1 мм.

Сопротивление одного витка — по средней длине витка бобины PQ32/30 (MLT ≈ 58 мм, оценка по типовым данным бобины этого типоразмера) и удельному сопротивлению меди при рабочей температуре ≈100 °C:

$$ {\Large P_{\text{Cu}} = I^2 \cdot R, \quad R = \rho \cdot \dfrac{L}{A} } ,~(2.5)$$

Таблица 2.4 — Потери в меди, на один трансформатор

| Обмотка | Витков | I, А | Ширина дорожки | R, мОм | P_Cu, Вт |
|:--|:--|:--|:--|:--|:--|
| Первичная | 23 | 6,6 | 2,1 мм (1 oz) | 413 | 18,0 |
| Вторичная | 6 | 25 | 6,5 мм (2 oz) | 17,2 | 10,7 |
| **Сумма на трансформатор** | | | | | **28,7** |
| **Сумма на два трансформатора** | | | | | **57,4** |

Ширина дорожек и допущение по MLT требуют подтверждения при трассировке платы; на стадии эскизного проекта это первое приближение.

**Потери в феррите.** Удельные потери N87 в опорной точке 100 кГц / 200 мТл / 100 °C — около 300 кВт/м³ (справочные данные производителя материала). Пересчёт на рабочую точку (65 кГц, Bmax = 217,5 мТл при номинальном U_ВВ) выполнен по степенной аппроксимации Штейнмеца с показателями, типичными для N87 в этом диапазоне (по частоте x ≈ 1,5, по индукции y ≈ 2,7):

$$ {\Large P_v = P_{v0} \cdot \left(\dfrac{f}{f_0}\right)^{x} \cdot \left(\dfrac{B_{\text{max}}}{B_0}\right)^{y} } ,~(2.6)$$

P_v = 300 × (65/100)^1,5 × (217,5/200)^2,7 ≈ **197 кВт/м³**. При Ve = 10 440 мм³ на трансформатор потери в сердечнике P_ф = 197 000 × 10440×10⁻⁹ ≈ **2,06 Вт**, на два трансформатора — **4,1 Вт**.

**Итог.** Суммарные потери двух трансформаторов на номинальной мощности: 57,4 Вт (медь) + 4,1 Вт (феррит) ≈ **61,5 Вт**, или 3,1 % от 2000 Вт передаваемой мощности. Потери в меди почти на порядок превышают потери в феррите — конструкция ограничена по току обмоток (печатная дорожка в окне), а не по индукции, что соответствует изначальной посылке о выборе сердечника по сечению, а не по объёму. Значение входит в общий баланс потерь наряду с проводимостью и коммутацией ключей (п. 2.1.1, п. 2.1.3) и уточняется в сводном расчёте КПД (п. 8.1–8.2).

Использованная методика — стандартное уравнение трансформатора для прямоугольного напряжения и площадь-произведение (Ae·Aw) как критерий выбора типоразмера, изложенные в общепринятой литературе по расчёту магнитных компонентов (например, McLyman, «Transformer and Inductor Design Handbook») и в прикладных материалах производителей ферритов (TDK); специальная статья или референс-дизайн для данного расчёта не привлекались — конкретный источник со стороны пользователя предоставлен не был.
