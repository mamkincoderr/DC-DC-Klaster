...
6. [Накопительные и фильтрующие элементы](#6-накопительные-и-фильтрующие-элементы)
   ...
   3. [Действующие токи пульсаций и выбор типов конденсаторов](#63-действующие-токи-пульсаций-и-выбор-типов-конденсаторов)
...
8. [Моделирование](#8-моделирование)
   ...
   2. [Проверка режимов в номинальной точке](#82-проверка-режимов-в-номинальной-точке)
   3. [Проверка на границах диапазона напряжений и мощности](#83-проверка-на-границах-диапазона-напряжений-и-мощности)
...
$$ {\Large P_v = P_{v0} \cdot \left(\dfrac{f}{f_0}\right)^{x} \cdot \left(\dfrac{B_{\text{max}}}{B_0}\right)^{y} } ,~(5.10)$$
...
**Принятые приборы.** Сторона ВВ: **TLP155E**, оптоизолированный драйвер с двухтактным выходом, по одному на ключ, четыре штуки. Сторона НВ: **EG2104**, полумостовой драйвер с бутстрепным питанием верхнего плеча, по одному на полумост, четыре штуки на два моста. Оба выбраны как наиболее доступные по цене приборы своих классов; достаточность их характеристик подтверждается расчётом ниже.

**Проверка стороны ВВ.** Драйвер TLP155E обеспечивает импульсный ток ±0,6 А, напряжение изоляции 3750 В действующего значения, устойчивость к синфазной помехе ±15 кВ/мкс. Транзистор NCE65TF099 имеет полный заряд затвора 55 нКл в худшем случае и заряд Миллера 11,5 нКл.

Таблица 5.7 — Проверка драйвера стороны ВВ
...
| Длительность плато Миллера | Q_gd/I = 11,5·10⁻⁹/0,6 | 19 нс |
| Полное время заряда затвора | Q_g/I = 55·10⁻⁹/0,6 | 92 нс, или 1,1 % полупериода |
| Затворный резистор | (15 − 5)/0,6 | 17 Ом |

Средний ток затвора на три порядка меньше импульсного, поэтому нагрузка на драйвер определяется исключительно импульсом. Время заряда затвора занимает около одного процента полупериода и форму напряжения на трансформаторе не искажает. Напряжение изоляции вдвое превышает испытательное значение, заданное техническим заданием.

**Проверка стороны НВ.** Драйвер EG2104 обеспечивает импульсный ток порядка 0,3 А при втекающем токе до 0,6 А и рассчитан на 600 В, что для шины 60 В даёт десятикратный запас. Транзистор AON6226 имеет полный заряд затвора 60 нКл в худшем случае и заряд Миллера 4,5 нКл.

Таблица 5.8 — Проверка драйвера стороны НВ

| Величина | Расчёт | Значение |
|:--|:--|:--|
| Средний ток затвора | Q_g·f = 60·10⁻⁹ × 60·10³ | 3,6 мА |
| Рассеяние на канал | Q_g·U_др·f = 60·10⁻⁹ × 12 × 60·10³ | 43 мВт |
| Длительность плато Миллера | Q_gd/I = 4,5·10⁻⁹/0,3 | 15 нс |
| Полное время заряда затвора | Q_g/I = 60·10⁻⁹/0,3 | 200 нс, или 2,4 % полупериода |

Меньший импульсный ток компенсируется вчетверо меньшим зарядом Миллера, поэтому напряжение на ключе переключается даже быстрее, чем на стороне ВВ. Полное время заряда затвора вдвое больше, но остаётся в пределах трёх процентов полупериода.

**Бутстрепное питание верхнего плеча.** Ёмкость выбирается из баланса заряда за полупериод, в течение которого верхний ключ открыт и конденсатор не подзаряжается:

$$ {\Large C_{б} \ge \dfrac{Q_g + I_{qbs} \cdot t + Q_{ls}}{\Delta U} } ,~(5.11)$$

При Q_g = 60 нКл, токе покоя верхнего канала 150 мкА за 8,33 мкс (1,25 нКл), заряде сдвига уровня 5 нКл и допустимой просадке 0,5 В требуется не менее 132 нФ. Принимается 1 мкФ, что даёт запас 7,5 раза и допускает работу при пониженной частоте. Бутстрепный диод выбирается быстрым, на напряжение не ниже 100 В.

**Изолированное питание стороны ВВ.** Оптодрайвер изолирует только сигнал, поэтому выходная часть каждого канала требует собственного питания. Нижние ключи моста имеют общий исток и питаются от одного источника; каждый верхний ключ требует отдельного гальванически развязанного источника. Потребление составляет около 50 мВт на канал, поэтому достаточно маломощного преобразователя с многообмоточным трансформатором либо готовых изолированных модулей на 0,25 Вт.
...
## 6.3 Действующие токи пульсаций и выбор типов конденсаторов
...
## 8.2 Проверка режимов в номинальной точке

## 8.3 Проверка на границах диапазона напряжений и мощности
