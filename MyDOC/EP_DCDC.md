...
**Параметры магнитопровода.** По даташиту TDK на PQ32/30 (сердечник B65879B, каркас B65880E) комплект из двух половин имеет эффективное сечение Ae = 153,8 мм² при минимальном сечении Amin = 127,5 мм², среднюю длину магнитной линии le = 67,8 мм, эффективный объём Ve = 10 440 мм³ и магнитный формфактор Σl/A = 0,441 мм⁻¹ при массе комплекта 57,4 г. Для материала N87 без зазора: индуктивность на квадрат числа витков AL = 4800 нГн с допуском +30/−20 %, эффективная магнитная проницаемость µe = 1700, удельные потери PV < 7,00 Вт на комплект в точке 200 мТл / 100 кГц / 100 °C, код заказа B65879B0000R087. Каркас односекционный: сечение окна намотки AN = 104 мм², средняя длина витка lN = 62 мм, сопротивление на квадрат числа витков AR = 21 мкОм, 12 выводов, код заказа B65880E2012D001. Диаметр центрального керна 13,45 мм, габарит сердечника 32 × 27,5 мм при высоте 30,35 мм. Допуск на AL в +30/−20 % относится к индуктивности намагничивания и определяет разброс параметров между двумя трансформаторами, что учитывается при проверке деления напряжений.

**Габаритная мощность.** Габаритная (типовая) мощность трансформатора это среднее арифметическое полных мощностей его обмоток:

$$ {\Large S_{\text{т}} = \dfrac{U_1 I_1 + U_2 I_2}{2} } ,~(2.1)$$

Каждая обмотка ВВ работает под половиной напряжения шины, то есть под 200 В в номинале, при токе 6,6 А, что даёт 1320 ВА. Обмотка НВ работает под 51,2 В при токе 25 А, то есть 1280 ВА. Отсюда S_т = (1320 + 1280)/2 = **1300 ВА**. Величина превышает передаваемые одним трансформатором 1000 Вт, поскольку габаритная мощность считается по полным, а не активным мощностям обмоток.

**Проверка размера сердечника.** Способность сердечника пропустить эту мощность выражается через произведение сечения на окно. Подставив в (2.1) напряжение обмотки из уравнения трансформатора и заменив ампер-витки произведением плотности тока на сечение меди, получаем

$$ {\Large S_{\text{т}} = 2 \cdot f \cdot B_{\text{max}} \cdot J \cdot K_u \cdot A_e \cdot A_w } ,~(2.2)$$

где двойка возникает из коэффициента формы прямоугольного напряжения (Kf = 4), делённого пополам согласно определению (2.1). Приняв типичные для фольговых и планарных обмоток Ku = 0,7 и J = 4 А/мм² при рабочей индукции 230 мТл, получаем 1241 ВА против требуемых 1300 ВА. Сердечник используется на пределе своих возможностей, запаса по габаритной мощности практически нет.

Формула (2.2) предполагает одну плотность тока для обеих обмоток, тогда как в принятой конструкции они различаются в тридцать раз: провод обмотки ВВ работает при 4 А/мм², фольга обмотки НВ при 132 А/мм². Фактическое заполнение окна медью тоже ниже принятого и составляет 0,38, поскольку фольга толщиной 35 мкм лежит на подложке платы толщиной 1 мм. Результат расчёта от этого не меняется, так как в (2.2) входит произведение J·Ku: принятое 2,8 А/мм² против фактического 2,9 А/мм². Прямая проверка размещения меди в окне выполнена ниже, после выбора числа витков.
...
|:--|:--|:--|:--|:--|:--|:--|
| 4 / 16 | 80 кГц | 260 мТл | 22 % | 18,8 Вт | 10,2 Вт | 28,9 Вт |
| 4 / 16 | 100 кГц | 208 мТл | 37 % | 18,8 Вт | 7,8 Вт | 26,5 Вт |
| **6 / 23** | **60 кГц** | **231 мТл** | **31 %** | **28,1 Вт** | **4,8 Вт** | **32,9 Вт** |
| 6 / 23 | 80 кГц | 173 мТл | 48 % | 28,1 Вт | 3,4 Вт | 31,5 Вт |
| 6 / 23 | 100 кГц | 139 мТл | 58 % | 28,1 Вт | 2,6 Вт | 30,7 Вт |

Сам по себе трансформатор тем лучше, чем выше частота. При постоянном числе витков индукция падает обратно пропорционально частоте, а показатель Штейнмеца по индукции (2,7) больше, чем по частоте (1,5), поэтому потери в феррите снижаются; меньшее число витков, которое допускает высокая частота, снижает и потери в меди. Будь трансформатор единственным потребителем, оптимум лежал бы на 100 кГц при четырёх витках, 26,5 Вт.

Но частоту задаёт не трансформатор, а ключи. Все составляющие потерь переключения пропорциональны частоте, и их в преобразователе двенадцать против двух магнитопроводов. Переход с 60 на 100 кГц экономит на трансформаторе 6 Вт и одновременно увеличивает потери переключения на 67 %, причём именно тот механизм, ради минимизации которого выбирались приборы с малым Qrr (п. 2.1.1). Количественная проверка этого баланса выполняется в п. 2.1.3.
...
**Размещение обмоток в окне.** Радиально доступно 6,5 мм, высота намотки по бобине B65880E равна 18,5 мм. Шесть плат толщиной 1 мм занимают 6 мм по высоте, оставляя 12,5 мм. Провод AWG15 с изоляцией имеет диаметр около 1,51 мм, значит в слое размещается 8 витков, а 23 витка требуют трёх слоёв и 4,5 мм по радиусу из имеющихся 6,5 мм. Суммарное заполнение окна составляет 75 %, свободными остаются около 2 мм по радиусу под межобмоточную изоляцию. Требуемое изоляционное расстояние определяется испытательным напряжением по техническому заданию и уточняется при разработке конструкции.
...
**Потери в меди.** Сечение витка обмотки НВ задано геометрией окна. Центральный керн имеет диаметр 13,45 мм, наибольший диаметр, помещающийся в окно, равен 26,4 мм (по фланцу штатной бобины), то есть радиально доступно (26,4 − 13,45)/2 = 6,5 мм. За вычетом технологического допуска 1 мм ширина меди составляет 5,5 мм при толщине фольги 35 мкм, что даёт сечение витка 0,19 мм². Провод обмотки ВВ берётся по плотности тока 4 А/мм² при I1 = 6,6 А: сечение 1,65 мм² (около AWG15). Сопротивление считается по средней длине витка (62 мм, даташит бобины) и удельному сопротивлению меди при ≈100 °C:

$$ {\Large P_{\text{Cu}} = I^2 \cdot R, \quad R = \rho \cdot \dfrac{L}{A} } ,~(2.4)$$

Таблица 2.6 — Потери в меди, на один трансформатор (N_НВ=6, N_ВВ=23)

| Обмотка | Витков | I, А | Сечение витка | R, мОм | P_Cu, Вт |
|:--|:--|:--|:--|:--|:--|
| ВВ (провод) | 23 | 6,6 | 1,65 мм² | 19,5 | 0,9 |
| НВ (6 плат по витку) | 6 | 25 | 5,5 × 0,035 = 0,19 мм² | 43,6 | 27,3 |
| **Сумма на трансформатор** | | | | | **28,2** |
| **Сумма на два трансформатора** | | | | | **56,4** |

Толщина фольги 35 мкм почти в девять раз меньше глубины скин-слоя на 60 кГц (309 мкм при 100 °C), поэтому ток по сечению фольги распределён равномерно и расчёт по сопротивлению постоянному току для обмотки НВ правомерен. Для провода обмотки ВВ это не так: диаметр AWG15 около 1,45 мм, то есть в пять раз больше скин-слоя, поэтому реальное сопротивление на рабочей частоте выше расчётного, а с учётом эффекта близости в 23 витках потери могут превысить табличные в два и более раза. Абсолютная величина при этом остаётся в пределах единиц ватт и на общий баланс не влияет, но уточнить её следует измерением на макете.

Потери обмотки НВ 27,3 Вт на трансформатор доминируют в балансе и целиком определяются толщиной фольги. Прямой резерв снижения состоит в использовании обеих сторон каждой платы с соединением слоёв переходными отверстиями: сечение витка удваивается, сопротивление и потери падают вдвое, до 13,7 Вт на трансформатор. Решение принимается на стадии разработки платы.
...
**Итог.** 56,4 Вт (медь) + 9,6 Вт (феррит) = **66 Вт** на два трансформатора, 3,3 % от 2000 Вт. При переходе на двустороннее исполнение витка обмотки НВ (см. выше) сумма снижается до 39 Вт, то есть 1,9 %. Потери в меди в шесть раз больше, чем в феррите, и почти целиком приходятся на обмотку НВ: конструкция ограничена сечением меди в окне, а не объёмом магнитопровода. Значение войдёт в общий баланс потерь вместе с проводимостью и коммутацией ключей (п. 2.1.1, п. 2.1.3) при расчёте КПД (п. 8.1–8.2).
