...
**Принятые приборы.** Сторона ВВ: **TLP155E**, оптоизолированный драйвер с двухтактным выходом, по одному на ключ, четыре штуки. Изоляция здесь обязательна: контроллер питается от батареи и привязан к отрицательной шине НВ, тогда как мост ВВ работает относительно отрицательной шины ВВ, гальванически отделённой трансформатором. Каждый ключ имеет собственный драйвер, поэтому мёртвое время формируется исключительно микроконтроллером.

Сторона НВ: **TNSG2186**, полумостовой драйвер с бутстрепным питанием верхнего плеча, по одному на полумост, четыре штуки на два моста. Гальваническая изоляция здесь не требуется, поскольку мосты НВ и контроллер привязаны к одной шине. Прибор выбран по двум причинам, кроме цены.
...
**Затворные резисторы.** Импульсный ток затвора определяется разностью между напряжением питания драйвера и напряжением плато Миллера, делённой на полное сопротивление цепи:

$$ {\Large I_{з} = \dfrac{U_{др} - U_{пл}}{R_{вых} + R_{вн} + R_{внутр}} } ,~(5.11)$$
...
$$ {\Large U_{пл} = U_{зи(пор)} + \dfrac{I_{с}}{g_{fs}} } ,~(5.12)$$

Для AON6226 при пороге 1,75 В, крутизне 90 См и токе ключа 17 А плато составляет 1,94 В. Для NCE65TF099 при пороге 3,5 В, крутизне порядка 20 См и токе 4,4 А плато составляет 3,7 В.

Таблица 5.10 — Затворные резисторы, номиналы ряда E24

| Сторона | U_др | U_пл | R_вых | R_внутр | **R_вн** | R_полн | I_з | Плато Миллера |
|:--|:--|:--|:--|:--|:--|:--|:--|:--|
| ВВ | 15 В | 3,7 В | 8 Ом | 1 Ом | **15 Ом** | 24 Ом | 0,47 А | 24 нс |
| НВ | 12 В | 1,9 В | 1,5 Ом | 1,4 Ом | **6,8 Ом** | 9,7 Ом | 1,04 А | 4,3 нс |

На стороне ВВ выходное сопротивление драйвера 8 Ом получено из паспортных 0,5 А при падении 4 В на выходном каскаде и составляет заметную долю цепи. Номинал 15 Ом выбран так, чтобы импульсный ток 0,47 А не выходил за типовые 0,5 А: меньшие номиналы 12 и 13 Ом дают 0,54 и 0,51 А, что приближается к предельным 0,6 А без выигрыша по скорости.

На стороне НВ драйвер по току не ограничивает, поэтому номинал выбирается по скорости выключения. Включение здесь мягкое в основной части диапазона (п. 4.6), а выключение жёстким остаётся всегда, и его длительность определяет потери. При 6,8 Ом плато Миллера длится 4,3 нс, что даёт около 0,9 Вт потерь выключения на восемь ключей. Уменьшение до 5,1 Ом сокращает их до 0,7 Вт, но повышает скорость нарастания напряжения и риск звона в затворной цепи, поэтому принимается 6,8 Ом как компромисс; окончательное значение уточняется по осциллограммам на макете.
...
$$ {\Large C_{б} \ge \dfrac{Q_g + I_{qbs} \cdot t_{полу} + Q_{ls}}{\Delta U} } ,~(5.13)$$
...
$$ {\Large Q_{б} = Q_g + I_{CC} \cdot t_{полу} + I_{обр} \cdot t_{полу} } ,~(5.14)$$
...
$$ {\Large C_{б} \ge \dfrac{Q_{б}}{\Delta U} } ,~(5.15)$$
...
$$ {\Large P_{вх} = k \cdot I_F \cdot U_F \cdot D } ,~(5.16)$$
...
$$ {\Large P_{вых} = k \cdot I_{CC} \cdot U_{др} + k \cdot Q_g \cdot U_{др} \cdot f } ,~(5.17)$$
