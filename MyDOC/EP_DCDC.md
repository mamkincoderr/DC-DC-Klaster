...
2. [Структурная схема преобразователя](#2-структурная-схема-преобразователя)
   1. [Состав и назначение функциональных узлов](#21-состав-и-назначение-функциональных-узлов)
...
показывает, насколько приведённое напряжение стороны НВ совпадает с напряжением стороны ВВ. При d = 1 приведённые напряжения сторон равны — это оптимальная рабочая точка любой изолирующей топологии: реактивная составляющая тока минимальна, условия коммутации наилучшие. Поскольку n постоянен, а Кп меняется, удержать d = 1 во всём диапазоне невозможно. Выбрав n = 7,81 из условия d = 1 в номинальной точке 400 В и 51,2 В, получаем d = 0,84…1,24 при полной мощности и d = 0,73…1,30 во всём диапазоне работоспособности.
...
### 1.1.2 Вариант 1 — двойной активный мост

Самая экономная по составу из двунаправленных изолирующих схем. Два одинаковых полных моста, между ними трансформатор и последовательная индуктивность — и больше ничего: ни выпрямителя, ни дросселя фильтра, ни разделительного конденсатора. Роль индуктивности может выполнять индуктивность рассеяния самого трансформатора.

![Двойной активный мост](PIC/topology-dab.svg)

Рисунок 1.1 — Двойной активный мост
...
ZVS обеспечивается током индуктивности: к моменту коммутации он должен течь в нужную сторону и иметь достаточную энергию для перезаряда выходных ёмкостей плеча. Условие по знаку тока даёт границы по углу: для моста ВВ φ > π·(d − 1)/(2d), для моста НВ φ > π·(1 − d)/2. В окне полной мощности, где d = 0,84…1,24, эти границы составляют 17,2° и 14,8°, а рабочий угол при 2000 Вт равен 35…37°. Запас двукратный, ZVS обеспечивается на обоих мостах во всём окне полной мощности.
...
### 1.1.4 Вариант 3 — мост с фиксированной скважностью и повышающий каскад на стороне ВВ

Здесь изоляция и регулирование разнесены по разным каскадам, и каждый занимается только своим делом.

![Мост с фиксированной скважностью и повышающий каскад](PIC/topology-dcx-boost.svg)

Рисунок 1.3 — Мост с фиксированной скважностью и повышающий каскад на стороне ВВ

Изолирующий каскад — два полных моста и трансформатор со скважностью 50 % без всякого регулирования, то есть трансформатор постоянного тока. Он ничего не стабилизирует: напряжение промежуточного звена просто следует за напряжением батареи, умноженным на n. Благодаря этому каскад всегда работает при d = 1 — в той самой оптимальной точке, которую два предыдущих варианта удержать не могут. Реактивная составляющая тока минимальна. Подмагничивание при этом не исчезает: скважность 50 % даёт нулевой вольт-секундный баланс лишь при идеально симметричных плечах, а реальная несимметрия сопротивлений открытых ключей и мёртвого времени накапливается так же, как в варианте 1, и требует той же коррекции по измеренному току.

Регулирующий каскад — обычный неизолированный повышающий преобразователь между промежуточным звеном и стороной ВВ. Он стабилизирует напряжение, задаёт передаваемую мощность, отрабатывает профиль заряда батареи и задаёт направление передачи энергии; изолирующий каскад собственной логики направления не содержит.

Коэффициент трансформации выбирается из условия n·U_НВ.макс ≤ U_ВВ.мин, откуда n ≤ 6. При n = 6 промежуточное звено работает в диапазоне 240…360 В, а наибольшее требуемое отношение напряжений регулирующего каскада составляет 430/240 = 1,79 при коэффициенте заполнения 0,44.
...
### 1.1.5 Вариант 4 — понижающая ячейка на стороне ВВ и раздельное регулирование направлений

Вариант воспроизводит силовой тракт серийного гибридного инвертора POW-HVM3.2H и ряда других недорогих инверторов китайского производства. От варианта 3 он отличается составом регулирующей ячейки: вместо двух ключей в ней один ключ и диод.

![Понижающая ячейка на стороне ВВ и двухтактный каскад НВ](PIC/topology-pow-hvm-charger.svg)

Рисунок 1.4 — Силовой тракт прототипа: понижающая ячейка на стороне ВВ

Регулирующая ячейка образована ключом VT5 в отрицательной шине ВВ, диодом VD1 и дросселем L1 в плюсовой шине. Конденсатора промежуточного звена нет: мост ВВ включён внутрь контура, образованного дросселем и диодом. На рисунке каскад НВ показан так, как он выполнен в прототипе, — по схеме со средней точкой; на анализ это не влияет (см. п. 1.2.1).

Отличие от варианта 3 в том, что двумя направлениями управляют разные узлы.

При **разряде** ведущим является каскад НВ. Он работает с изменяемой скважностью, L1 служит дросселем выходного фильтра, мост ВВ — выпрямителем, VT5 удерживается открытым и только проводит ток. Напряжение шины U_ВВ = 2·D·n·U_НВ, где D — скважность плеча каскада НВ, не превышающая 0,5.

При **заряде** ведущей становится ячейка. VT5 работает в режиме ШИМ, L1 — дроссель понижающего преобразователя, VD1 — его обратный диод, мост ВВ инвертирует напряжение звена в трансформатор, ключи стороны НВ выпрямляют. Напряжение батареи U_НВ = D′·U_ВВ/n, где D′ — коэффициент заполнения VT5.
...
| Показатель | Вариант 1 (DAB) | Вариант 2 (CLLLC) | Вариант 3 (мост + повышающий каскад) | Вариант 4 (мост + понижающая ячейка) |
|:--|:--|:--|:--|:--|
| Число преобразований энергии | 1 | 1 | 2 | 2 |
| Число ключей в тракте передачи энергии | 8 | 8 | 10 | 9 |
| Реактивные элементы | дроссель | 2 дросселя и 2 конденсатора | дроссель регулирующего каскада | дроссель регулирующей ячейки |
| Управление передаваемой мощностью | фазовым сдвигом, один контур | частотой, один контур | коэффициентом заполнения ячейки, один контур | два контура: скважность каскада НВ при разряде, скважность ячейки при заряде |
| Отработка изменения Кп | не отрабатывается, d уходит от единицы | уходом от резонанса | регулирующим каскадом, изолирующий остаётся при d = 1 | оба направления понижающие, требуемого n не существует (см. п. 1.2) |
| ZVS в окне полной мощности | обеспечивается на обоих мостах, запас по углу двукратный | ZVS моста ВВ только выше резонанса | обеспечивается в изолирующем каскаде, отсутствует в регулирующем | не обеспечивается ни в одном каскаде |
| ZVS при мощности 200 Вт | не обеспечивается | обеспечивается током намагничивания | обеспечивается в изолирующем каскаде | не обеспечивается |
| ZCS моста НВ | не достигается | на резонансе и ниже | не достигается | не достигается |
| Реактивная и циркулирующая составляющие тока | растут на 5…6 % на краях окна полной мощности | растут вне резонанса | присутствуют постоянно, от заниженной индуктивности намагничивания | растут при снижении скважности каскада НВ |
| Наиболее нагруженный элемент стороны НВ | ключи моста | резонансный конденсатор, 40…60 А | ключи моста | ключи моста |
| Дополнительные элементы стороны ВВ | нет | нет | дроссель и конденсатор промежуточного звена, 5,6…8,3 А | дроссель ячейки; конденсатора промежуточного звена нет |
| Подмагничивание трансформатора | накапливается при несимметрии плеч, требуется коррекция по току | блокируется резонансным конденсатором | то же, что в варианте 1 | то же, что в варианте 1 |
| Частота коммутации | постоянная | переменная | постоянная, кроме режима ZVS регулирующего каскада | постоянная |
| Чувствительность к разбросу параметров | низкая | высокая, две резонансные частоты | низкая | низкая |
| КПД в номинальной точке | 95…96 % | 96…97 % | 93,5…95 % | 93,5…95 % |
| КПД на краях окна полной мощности | 94…95 % | 93…95 % | 93,5…95 % | 93,5…95 % |
| КПД при 200 Вт на краю диапазона работоспособности | около 92 % | 90…93 % | около 92 % | около 92 % |
| Устранение слабого места | второй управляющей переменной, без изменения силовой схемы | изменением параметров резонансного контура | изменением режима работы регулирующего каскада | в пределах топологии не устраняется |

Число каскадов итоговый КПД не определяет: два каскада перемножают свои коэффициенты, но каждый из них может быть выше, чем у однокаскадной схемы, работающей вне оптимума. Однако у варианта 3 обе составляющие потерь — циркулирующий ток изолирующего каскада и обратное восстановление в регулирующем — присутствуют постоянно, поэтому его пологая характеристика проходит ниже, чем характеристика варианта 1 в окне полной мощности.

**Выбор: вариант 1, двойной активный мост.**

Решающим оказалось разделение диапазонов в техническом задании. Полная мощность требуется в окне 46…57 В, где d = 0,84…1,24, а границы сохранения ZVS составляют 17,2° и 14,8° против рабочего угла 35…37°. Там, где передаётся вся мощность, обеспечивается ZVS на обоих мостах, а рост действующего тока на краях окна не превышает 6 %. Расширенный диапазон 40…60 В, где ZVS теряется, требует всего 200 Вт, и потери на перезаряд выходных ёмкостей составляют там около 5 Вт.

Вариант 2 при этом же условии выигрывает только в узкой полосе вокруг резонанса, а требуемое изменение Кп в 1,48 раза из неё выводит. Кроме того, резонансный конденсатор стороны НВ на 40…60 А остаётся самым нагруженным элементом схемы независимо от режима, а переменная частота коммутации не позволяет рассчитать фильтр электромагнитной совместимости и магнитные элементы на одну частоту.

Вариант 3 обеспечивает наиболее ровный КПД по диапазону, но его уровень ниже, поскольку обе дополнительные составляющие потерь действуют постоянно, а не только на краях. Он остаётся резервным решением на случай, если измеренный КПД варианта 1 на краях окна полной мощности не удовлетворит требованиям технического задания.

Вариант 4 двунаправлен и регулируется в обе стороны, поэтому по управляемости мощностью вариантам 1–3 он не уступает. Его ограничение количественное. Регулирование в обоих направлениях способно только снижать напряжение относительно значения, заданного коэффициентом трансформации: при разряде U_ВВ = 2·D·n·U_НВ ≤ n·U_НВ, при заряде U_НВ = D′·U_ВВ/n ≤ U_ВВ/n. Первое требует n ≥ 430/46 = 9,35, второе — n ≤ 360/57 = 6,32. Такого коэффициента трансформации не существует. Прототип с этим не сталкивается, поскольку его шина не нормирована — 300…450 В там результат, а не требование; здесь диапазон 360…430 В задан инверторным каскадом.

Второе отличие структурное. Регулирующие узлы разнесены по разные стороны гальванической развязки, поэтому нужны два контура регулирования с собственными датчиками, а смена направления означает передачу управления с одной стороны на другую. Вариант 3 обходится одним контуром: изолирующий каскад всегда работает при фиксированной скважности, всё регулирование ведёт ячейка.

По КПД варианты 3 и 4 на этой стадии неразличимы. У варианта 4 при разряде ячейка не преобразует энергию, а только проводит ток, но каскад НВ работает при скважности ниже предельной, что повышает действующие токи; у варианта 3 ячейка преобразует энергию в обоих направлениях, зато изолирующий каскад всегда остаётся при d = 1. Различие между ними лежит не в КПД, а в структуре регулирования.

Существенно и то, как устраняются слабые места. У варианта 1 потеря ZVS при малой мощности и рост реактивного тока при отклонении d от единицы устраняются введением второй управляющей переменной — дополнительного фазового сдвига внутри моста, — то есть правкой программы управления без изменения силовой схемы. Подмагничивание трансформатора устраняется там же, коррекцией по измеренному току. У варианта 2 расширение рабочего диапазона потребовало бы пересчёта резонансного контура и переделки магнитных элементов, у варианта 3 — изменения режима работы регулирующего каскада с потерей постоянной частоты коммутации.
...
## 2.1 Состав и назначение функциональных узлов
