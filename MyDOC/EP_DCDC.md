...
Формула (2.2) предполагает одну плотность тока для обеих обмоток, тогда как здесь они различаются в тридцать раз: провод обмотки ВВ работает при 4 А/мм², фольга обмотки НВ при 132 А/мм². В расчёт входит произведение Ku·J, то есть ампер-витки на единицу сечения окна; для принятой конструкции оно равно 301,8 А / 138 мм² = 2,2 А/мм² против принятых в оценке 2,8 А/мм². Прямая проверка размещения меди в окне выполнена ниже, после выбора числа витков.
...
**Размещение обмоток в окне.** Радиально доступно 6,5 мм при высоте окна 21,3 мм. Шесть плат толщиной 1 мм занимают 6 мм по высоте, оставляя 15,3 мм. Провод AWG15 с изоляцией имеет диаметр около 1,51 мм, значит в слое размещается 10 витков, а 23 витка требуют трёх слоёв и 4,5 мм по радиусу из имеющихся 6,5 мм. Суммарное заполнение окна составляет 74 %, свободными остаются около 2 мм по радиусу под межобмоточную изоляцию. Требуемое изоляционное расстояние определяется испытательным напряжением по техническому заданию и уточняется при разработке конструкции.
...
**Потери в меди.** Сечение витка обмотки НВ задано геометрией окна: при радиальной глубине 6,5 мм и технологическом допуске 1 мм ширина меди составляет 5,5 мм, что при толщине фольги 35 мкм даёт сечение витка 0,19 мм². Провод обмотки ВВ берётся по плотности тока 4 А/мм² при I1 = 6,6 А: сечение 1,65 мм² (около AWG15). Сопротивление считается по средней длине витка 62 мм и удельному сопротивлению меди при ≈100 °C:
