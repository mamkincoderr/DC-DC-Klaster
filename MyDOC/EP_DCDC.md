...
Помимо статических потерь проводимости, для мостовой схемы с жёсткой коммутацией встречного диода решающим параметром служит заряд обратного восстановления Qrr и время trr: при малом фазовом угле, вблизи границы ZVS (см. п. 1.1.5), диод плеча восстанавливается под полным напряжением шины, и потери на восстановление растут с частотой линейно. Второй по значимости параметр — заряды затвора и выходные ёмкости (Qg, Coss, Crss), определяющие потери на переключение и требования к драйверу. Третий — запас по напряжению сверх рабочего диапазона: индуктивность рассеяния трансформатора даёт выброс напряжения при каждой коммутации, и минимального запаса «на бумаге» недостаточно.
...
Приборы на 80 В отсекаются по запасу напряжения: при максимуме батареи 60 В запас составляет 1,33×, что для моста с индуктивностью рассеяния трансформатора мало — в разделе принимается порог 100 В. Из оставшихся двух CRSM038N10N4 при 3,0 мОм вдвое превосходит AON6226 по проводимости; на восемь ключей стороны НВ разница составляет около 37 Вт при разнице в цене комплекта порядка 40 грн. **Выбор при приоритете КПД: CRSM038N10N4; при приоритете цены — резервный вариант AON6226.** Оба — в корпусе DFN 5×6: теплоотвод идёт через массив переходных отверстий в плате, а не через радиатор, что становится входным условием для теплового расчёта (п. 10.3) и компоновки платы (п. 10.4).

Для NCE65TF099 и CRSM038N10N4 производитель не приводит RDS(on) при верхней рабочей температуре; расчёты раздела 4 ведутся по множителям ×2,5 и ×1,8 соответственно, как принятое допущение до получения данных от производителя или прямого измерения.
