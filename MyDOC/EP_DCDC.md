...
**Обмотки.** Сторона НВ: мало витков и большой ток, поэтому обмотку предлагается выполнить печатной дорожкой в окне сердечника. Это удобно и повторяемо, без ручной намотки толстым проводом. Сторона ВВ наоборот, много витков при небольшом токе; для неё предлагается провод, поскольку такое число витков дорожкой без многослойной платы не разместить.

**Плотность тока.** Дорожка в окне на двухсторонней плате практически ограничена по току примерно 30 А (эмпирическая формула IPC-2221 для наружного слоя даёт близкую цифру, см. ниже). Суммарный ток обмотки НВ 50 А в одну такую дорожку не укладывается, что и подтверждает разделение на два трансформатора: по 25 А на обмотку, запас 17 % от предела. Обмотки ВВ включены последовательно в диагональ моста ВВ (один и тот же ток 6,6 А через обе, напряжение делится пополам), обмотки НВ каждая на свой мост НВ. Мосты запараллелены по LV+/LV−, деление тока между трансформаторами самовыравнивается схемой, отдельного контроля не требует.
...
где Ku ≈ 0,3 это коэффициент заполнения окна медью с учётом изоляции и зазоров, J плотность тока обмотки. Для провода при обычной плотности 4 А/мм² и Bmax = 230 мТл (типовое рабочее значение для N87 на этой частоте, порядок величины ниже насыщения ~390 мТл при 100 °C) формула (2.1) даёт около 530 ВА, заметно меньше требуемых 1250 ВА. Разрыв не говорит о нехватке сердечника: формула предполагает одну плотность тока на обе обмотки, а у нас обмотка НВ выполнена печатной дорожкой, где практическая плотность тока на порядок выше провода (для 25 А и принятой ширины дорожки около 55 А/мм², расчёт ниже). Именно это и даёт разделение обмоток по способу изготовления: печатная дорожка компактно закрывает высокий ток стороны НВ, провод умеренный ток и много витков стороны ВВ. Точная проверка по реальным токам и напряжению (см. «Потери в меди» ниже) даёт S_т = 1300 ВА, что и принимается как расчётное значение.
...
**Витки.** Коэффициент трансформации одного трансформатора вдвое меньше найденного в п. 1.1.1 коэффициента n = 7,8125 для системы в целом (формула (1.1)): каждая обмотка ВВ видит только половину напряжения ВВ, а обмотка НВ должна воспроизвести полное напряжение НВ, то есть n_т = n/2 = 3,906. Число витков находится по уравнению трансформатора для прямоугольного напряжения:

$$ {\Large U = 4 \cdot f \cdot N \cdot B_{\text{max}} \cdot A_e } ,~(2.2)$$

Отправная точка это обмотка НВ, поскольку у неё меньше витков и округление до целого числа заметнее. Рассмотрены три варианта:
...
где k = 0,048, A сечение в мил². Для I2 = 25 А, медь 2 oz, сечение получается 0,46 мм² при ширине около 6,5 мм (укладывается в окно бобины с учётом зазоров). Провод обмотки ВВ берётся по плотности тока 4 А/мм² при I1 = 6,6 А: сечение 1,65 мм² (около AWG15). Сопротивление витка считается по реальной длине витка бобины (62 мм, даташит) и удельному сопротивлению меди при ≈100 °C:
...
**Потери в феррите.** Удельные потери N87 в опорной точке 100 кГц / 200 мТл / 100 °C составляют около 300 кВт/м³ (данные производителя материала). Пересчёт на рабочую точку (60 кГц, Bmax = 231 мТл) ведётся по степенной аппроксимации Штейнмеца (показатель по частоте x ≈ 1,5, по индукции y ≈ 2,7, типичные для N87 в этом диапазоне):

$$ {\Large P_v = P_{v0} \cdot \left(\dfrac{f}{f_0}\right)^{x} \cdot \left(\dfrac{B_{\text{max}}}{B_0}\right)^{y} } ,~(2.5)$$

P_v ≈ 206 кВт/м³. При Ve = 10 440 мм³ это 2,15 Вт на трансформатор, 4,3 Вт на два.
...
Методика взята стандартная: уравнение трансформатора для прямоугольного напряжения и площадь-произведение как критерий сопоставления мощности с размером сердечника, по общей литературе (McLyman, «Transformer and Inductor Design Handbook») и прикладным материалам TDK. Отдельной статьи или референс-дизайна под этот расчёт не привлекалось.
