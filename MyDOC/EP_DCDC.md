...
Формула (2.2) предполагает одну плотность тока для обеих обмоток, тогда как здесь они различаются в шестнадцать раз: провод обмотки ВВ работает при 4 А/мм², фольга обмотки НВ при 65 А/мм². В расчёт входит произведение Ku·J, то есть ампер-витки на единицу сечения окна; для принятой конструкции оно равно 301,8 А / 138 мм² = 2,2 А/мм² против принятых в оценке 2,8 А/мм². Прямая проверка размещения меди в окне выполнена ниже, после выбора числа витков.
...
|:--|:--|:--|:--|:--|:--|:--|
| 4 / 16 | 80 кГц | 260 мТл | 22 % | 9,7 Вт | 10,2 Вт | 19,8 Вт |
| 4 / 16 | 100 кГц | 208 мТл | 37 % | 9,7 Вт | 7,8 Вт | 17,5 Вт |
| **6 / 23** | **60 кГц** | **231 мТл** | **31 %** | **14,5 Вт** | **4,8 Вт** | **19,3 Вт** |
| 6 / 23 | 80 кГц | 173 мТл | 48 % | 14,5 Вт | 3,4 Вт | 17,9 Вт |
| 6 / 23 | 100 кГц | 139 мТл | 58 % | 14,5 Вт | 2,6 Вт | 17,1 Вт |

Разброс по суммарным потерям между всеми проходными вариантами укладывается в 2,7 Вт, то есть 0,1 % от передаваемой мощности, поэтому выбор определяется не ими. Существенно, что выбранный вариант 6 витков на 60 кГц по сумме потерь не уступает варианту 4 витка на 80 кГц (19,3 против 19,8 Вт), хотя работает на более низкой частоте.

Формально минимум лежит на 100 кГц при четырёх витках, 17,5 Вт. Причина в том, что при постоянном числе витков индукция падает обратно пропорционально частоте, а показатель Штейнмеца по индукции (2,7) больше, чем по частоте (1,5), поэтому потери в феррите с ростом частоты снижаются. Но выигрыш в 2,2 Вт относительно выбранного варианта не окупается: частоту задаёт не трансформатор, а ключи, которых в преобразователе двенадцать против двух магнитопроводов, и все составляющие их потерь переключения пропорциональны частоте. Переход с 60 на 100 кГц увеличил бы их на 67 %, причём именно тот механизм, ради минимизации которого выбирались приборы с малым Qrr (п. 2.1.1). Количественная проверка выполняется в п. 2.1.3.
...
**Сечение и сопротивление обмотки НВ.** Каждый виток выполнен на отдельной двухсторонней плате: медь нанесена с обеих сторон и соединена по всей длине витка переходными отверстиями, так что обе стороны работают как один проводник. Ширина меди определена глубиной окна за вычетом технологического допуска: 6,5 − 1 = 5,5 мм. Толщина фольги 35 мкм на сторону.

| Величина | Значение |
|:--|:--|
| Ширина меди | 5,5 мм |
| Толщина фольги | 35 мкм на сторону |
| Сечение одной стороны | 5,5 × 0,035 = 0,193 мм² |
| **Сечение витка (две стороны)** | **0,385 мм²** |
| Средняя длина витка | 62 мм |
| Удельное сопротивление меди при 100 °C | 2,26·10⁻⁸ Ом·м |
| **Сопротивление витка** | **3,63 мОм** |
| Сопротивление обмотки (6 витков) | 21,8 мОм |
| **Потери при 25 А** | **13,6 Вт** |

Плотность тока в витке составляет 25 / 0,385 = 65 А/мм², что на порядок выше обычных для провода 4 А/мм². Допустимость такой плотности определяется не сечением, а теплоотводом: потери на виток равны 2,27 Вт при площади витка 341 мм² на сторону, то есть 0,67 Вт/см². Медь припаяна к диэлектрику платы, отводящему тепло по всей площади витка к выходящей за пределы сердечника части платы. Тепловой расчёт этого пути выполняется в п. 8.3; при неудовлетворительном результате сечение наращивается увеличением толщины фольги до 70 мкм на сторону.

Вытеснение тока в фольге отсутствует. Глубина скин-слоя на 60 кГц при 100 °C равна 309 мкм, толщина фольги 35 мкм, отношение Δ = 0,11. По формуле Доуэлла для двенадцати медных слоёв коэффициент увеличения сопротивления составляет 1,003, то есть расчёт по постоянному току даёт погрешность в доли процента. Это прямое следствие выбранной конструкции: тонкая фольга исключает и поверхностный эффект, и эффект близости, тогда как сплошной проводник того же сечения на этой частоте использовался бы лишь частично.

В расчёте не учтены сопротивление переходных отверстий между сторонами платы и сопротивление пяти соединений между шестью платами. При качественном исполнении их вклад составляет единицы процентов, но проверить его следует измерением на макете: при 25 А каждый лишний миллиом добавляет 0,6 Вт.

**Потери в меди.** Провод обмотки ВВ берётся по плотности тока 4 А/мм² при I1 = 6,6 А: сечение 1,65 мм² (около AWG15). Сопротивление обеих обмоток считается по средней длине витка 62 мм:
...
| НВ (6 двухсторонних плат) | 6 | 25 | 0,385 мм² | 21,8 | 13,6 |
| **Сумма на трансформатор** | | | | | **14,5** |
| **Сумма на два трансформатора** | | | | | **29,0** |

Для провода обмотки ВВ расчёт по постоянному току, в отличие от фольги, занижен: диаметр AWG15 около 1,45 мм, то есть в пять раз больше скин-слоя, и с учётом эффекта близости в трёх слоях потери могут превысить табличные вдвое. Абсолютная величина при этом остаётся ниже ватта и на баланс не влияет, но уточнить её следует измерением.
